...
// Your code goes here
hiddenCodeArray = hiddenCode.split('');
console.log(hiddenCodeArray);
let code = "";
for (var i = 0; i < hiddenCodeArray.length; i++) {
    if (hiddenCodeArray[i] != "X") {
        if ((parseFloat(hiddenCodeArray[i]) % 2) != 0) {
            code = code + hiddenCodeArray[i];
        }
    }
}
console.log(code);
